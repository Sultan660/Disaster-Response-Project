...
df = pd.read_sql_table('MessagesDataSet', engine)
...
def index():
    ...
    genre_counts = df.groupby('genre').count()['message']#df.groupby('genre').count()['message']
    genre_names = list(genre_counts.index)   #list(genre_counts.index)
